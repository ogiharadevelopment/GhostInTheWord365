...
function updatePremiumStatus(isPremium) {
    ...
    // ローカルストレージに保存
    localStorage.setItem('formatManagerPremium', isPremium.toString());
...
const texts = {
    ja: {
        ...
        premiumUpgrade: '有料版にアップグレード',
        premiumStatus: '有料版',
        freeStatus: '無料版',
        formatLimitReached: '保存可能な書式数に達しました',
        formatLimitMessage: '無料版では3個まで保存できます。古い書式が削除されます。',
        upgradeMessage: '無制限で保存するには有料版にアップグレードしてください',
        japanese: '日本語',
        english: 'English'
    },
    en: {
        ...
        freeStatus: 'Free',
        formatLimitReached: 'Format limit reached',
        formatLimitMessage: 'Free version allows up to 3 formats. Old formats will be deleted.',
        upgradeMessage: 'Upgrade to Premium for unlimited format storage',
        japanese: '日本語',
        english: 'English'
    }
};
...
function initializeApp() {
    ...
    try {
        ...
        console.log('Step 3: Language setup');
        // 言語設定は既に初期化時に設定済み
        console.log('Current language:', currentLanguage);
        
        console.log('Step 4: UI update');
        // UIの初期化
        updateUI();
        
        console.log('Step 5: Event listeners setup');
        // イベントリスナーの設定
        setupEventListeners();
        ...
        console.log('Step 6: Saved formats loading');
        // 保存された書式の読み込み
        loadSavedFormats();
        
        console.log('Step 7: Selection change handler');
        // 選択変更の監視
        try {
        ...
        console.log('Step 8: Initial format update');
        // 初期表示
        updateCurrentFormat();
        
        console.log('Step 9: Synthetic click setup');
        // 疑似クリックイベントの設定
        setupSyntheticClick();
...
function performSave(key, isOverwrite = false) {
    try {
        ...
        // 無料版で制限に達している場合の処理
        if (!isPremiumUser && Object.keys(savedFormats).length >= maxFormats) {
            // 古い書式を削除（タイムスタンプ順）
            const formatEntries = Object.entries(savedFormats);
            formatEntries.sort((a, b) => new Date(a[1].timestamp) - new Date(b[1].timestamp));
            
            const oldestKey = formatEntries[0][0];
            delete savedFormats[oldestKey];
            console.log('🗑️ Removed oldest format:', oldestKey);
            
            const t = texts[currentLanguage];
            showMessage(`${t.formatLimitMessage} (${oldestKey}を削除)`, 'info');
        }
...
function updateSavedFormatsList() {
    ...
    // 制限情報を追加
    const t = texts[currentLanguage];
    const currentCount = Object.keys(savedFormats).length;
    const limitInfo = !isPremiumUser ? 
        `<div style="font-size: 10px; color: #605e5c; padding: 5px; border-top: 1px solid #edebe9; text-align: center;">
            ${currentCount}/${maxFormats} ${t.formatLimitReached}
        </div>` : '';
    
    savedFormatsList.innerHTML = html + limitInfo;
...
window.togglePremium = function() {
    ...
    console.log('Premium status toggled:', isPremiumUser);
};
